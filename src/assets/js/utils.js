...
export const formatDateTo12HoursTime = function(date) {
  if (!(date instanceof Date))
    throw new Error(`Date value expected, got ${date.constructor.name}`)
...
export const assetSource = (path) => require(`../${path}`)
...
export class TypingAnim {
  ...
  _type() {
    ...
    this.elem.textContent = this.msg.substr(0, this._msgLength++)
  ...
  erase() {
    this._msgLength = this.elem.textContent.length

<template>
  ...
  >
    ...
    <div ref="history" :class="$style.items" @click="focusTerminalInput">
      <REPL
        v-for="(item, i) in historyItems"
        :key="i"
        :prop-input="item.input"
        :prop-output="item.output"
        :pwd="item.pwd"
      />
      <REPL
        ref="cli"
...
import { CommandNotFoundError } from '@js/exceptions.js'
import REPL from './REPL.vue'
import { navigation, terminal } from '~/store/navigation.js'
...
export default Vue.extend({
  ...
  methods: {
    ...
    getCommandPromise(cmd, args) {
      const envs = this.environmentVars
    ...
    onSubmitInput(words) {
      const cmd = words.splice(0, 1)[0].trim()
